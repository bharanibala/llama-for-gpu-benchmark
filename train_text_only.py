...
def get_model_and_tokenizer(model_path, ft_layers, lora_config):
    ...
    # tokenizer = AutoProcessor.from_pretrained("unsloth/Llama-3.2-11B-Vision-Instruct")
    # model = AutoModelForVision2Seq.from_pretrained("unsloth/Llama-3.2-11B-Vision-Instruct")
...
def get_dataset(task):
    if task == "informative":
        ...
        ds = load_dataset('json', data_files={'train': '../CrisisMMD_Modified/informative/text_only.jsonl'}) # /home/b/bharanibala/mllmd/Fine-tune-Multimodal-LLM-for-CrisisMMD-main/Anh-New/test_results/informative/Test-23/Llama-3.2-11B-Info-Oneshot-Text-Only.json'}) #processed_dataset/informative_train.json'})
        dataset = ds["train"]

        #dataset_loader = DataLoader(ds["train"], batch_size=4, shuffle=False)

        # test_data_path = "/home/b/bharanibala/mllmd/CrisisMMD_Modified/informative/text_only.json"
        # dataset = pd.read_json(test_data_path, lines=False, encoding="utf-8", orient="index")

        instruction = """
        You are an data expert in crisis management with many years of experience.
...
#        converted_dataset_2 = [to_device_and_pick(batch) for batch in dataset]

        converted_dataset = [convert_to_conversation_text_only(instruction, row) for row in dataset]
...
def start_training(model, tokenizer, data_collator, converted_dataset, learning_rate, epoch, batch_size, output_dir, gradient_accumulation_steps, logging_steps):
    ...
    trainer = SFTTrainer(
        model = model,
        tokenizer = tokenizer,
        data_collator = UnslothVisionDataCollator(model, tokenizer), # data_collator, # Must use!
